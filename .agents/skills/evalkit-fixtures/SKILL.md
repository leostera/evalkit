...
2. Give each fixture a stable lowercase kebab-case ID. Choose visibility deliberately: `candidate` files may be read by the agent/model and are snapshotted into reports; `evaluator` files are private to trusted adapter/scorer code, but still remain in local `_evalkit-sandbox/`.
3. Use `directory(fixtureId, src, { dst?, visibility? })` to copy a tree; default destination is its basename and default visibility is candidate. Use `file(fixtureId, src, { dst, visibility })` for a file; `inlineFile(fixtureId, relativePath, text, visibility)` for generated text; `dynamic(fixtureId, context => fixture | fixture[])` for per-trial generated inputs (async is allowed). Dynamic fixture context includes `trialIndex`, `metadata.randomSeed`, and optional `parameters`; return actual fixtures with their own IDs.
