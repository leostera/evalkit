...
1. Decide whether to reuse `piAgent()` for the local Pi CLI or implement `defineAgent({ identity, runtimes, start })` for a different agent. `agentsSdk()` currently throws; do not claim it is a usable remote adapter.
