...
This is a Bun workspace without a dedicated scaffold. `packages/evalkit` builds the single `@leostera/evalkit` artifact; the manual GitHub Packages workflow has not published it yet. The `@evalkit/*` packages are internal to the workspace. Read the Markdown guide `../../../www/src/content/docs/index.md` and topic pages under `../../../www/src/content/docs/manual/` for the user workflow; consult the implementation when behavior matters: `../../../packages/core/src/index.ts`, `../../../packages/runner/src/index.ts`, `../../../packages/cli/src/index.ts`, and `../../../examples/starter/` (paths relative to this skill directory). Check `git status --short` first and preserve unrelated changes.
