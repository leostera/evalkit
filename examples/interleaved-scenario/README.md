...
Both evals are discovered from default exports without a config. Reports are local under `_evalkit-results/` (v3 manifests and checkpoint results); trial workspaces remain under `_evalkit-sandbox/`, both ignored by Git. `src/run.test.ts` runs each eval in a temporary project report root and verifies evidence, skipped steps, and the combined pass gate. Checkpoints do **not** provide enforced timeouts, cancellation, or a process-wide egress guard.
