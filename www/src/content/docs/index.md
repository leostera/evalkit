...
Today `user(...)`, deterministic `check(...)`, and observed `expectToolCall(...)` transcript steps execute, along with post-session `predicate(...)` scorers. Judge scorers, remote Agents SDK transport, enforced timeouts, and standalone project scaffolding are not yet available. The single package has an external build but its first GitHub Packages release has not yet happened; see [installation notes](/docs/manual/project-structure/).
