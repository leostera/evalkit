...
- **Invalid score / scorer error:** Return a finite value between 0 and 1; `evidence` must be JSON-serializable. Predicate exceptions are captured as failed scores; other scorers can still execute. With a failed AUT, only final predicates or judges marked `supportsPartial: true` can run if a context/workspace exists. [Partial errors](/docs/manual/scoring-and-evals/#partial-execution-and-errors)
- **Missing eval `judge` / judge error:** Any inline or final `judge(...)` rule requires a separate eval-level `judge` agent; the AUT is not implicitly reused. The judge must emit a JSON verdict in `completed.output` or its assistant message. A missing/invalid verdict or failed judge session is a checkpoint execution error inline or a final scorer error. Judge usage appears only if the judge agent emits it; cost is not measured automatically. [Judges](/docs/manual/scoring-and-evals/#judge-agents)
...
- **No reports or a running entry with no details:** `serve-dashboard` reads the current project's `reportDir`. A trial may still be in progress, or a process may have ended before finalizing summaries. Confirm `--config` and project root, and inspect the report tree. The dashboard lists compatible v2 and v3 manifests; it does not migrate v1 files.
