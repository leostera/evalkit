...
export function parseRunArgs(args: string[]) {
  const { values, positionals } = parseArgs({ args, allowPositionals: true, strict: true, options: {
    config: { type: 'string' }, eval: { type: 'string', multiple: true }, model: { type: 'string', multiple: true }, mode: { type: 'string', multiple: true },
    select: { type: 'string', multiple: true }, param: { type: 'string', multiple: true },
    'max-tokens': { type: 'string' }, 'chat-timeout-ms': { type: 'string' }, 'turn-budget': { type: 'string' },
    concurrency: { type: 'string' }, trials: { type: 'string' }, json: { type: 'boolean' },
    'dry-run': { type: 'boolean' }, all: { type: 'boolean' }, local: { type: 'boolean' },
  } });
  ...
  for (const [flag, key] of [['max-tokens','maxTokens'], ['chat-timeout-ms','chatTimeoutMs'], ['turn-budget','turnBudget']] as const) {
  ...
  for (const key of ['model', 'mode'] as const) {
    if (values[key]) selection[key] = values[key].flatMap(value => value.split(','));
  }
  for (const value of values.select ?? []) {
    ...
    try { choice = JSON.parse(value.slice(index + 1)); } catch { choice = value.slice(index + 1); }
  ...
  return { values, positionals, parameters, selection, concurrency: integer('concurrency'), trials: integer('trials') };
}

export async function runProjectCommand(command: string, args: string[], cwd = process.cwd()): Promise<void> {
  ...
  for (const [axis, choices] of Object.entries(parsed.selection)) {
    if (axis in matrix.parameters) (selection.parameters as Record<string, JsonValue[]>)[axis] = choices;
  ...
  const summary = await Effect.runPromise(runMatrix(matrix, {
    selection,
