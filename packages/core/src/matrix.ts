...
export function defineEvalMatrix<const T extends EvalMatrixDefinition>(
  ...
  function select(selection: MatrixSelection = {}) {
    for (const key of Object.keys(selection.parameters ?? {})) {
      if (!(key in definition.parameters))
        throw new Error(`Unknown matrix axis: ${key}`);
    }
    ...
    const selectedAxes = axes.map(([axis, choices]) => {
      ...
      for (const key of keys)
        if (!choices.some((v) => canonicalParameters(v) === key))
          throw new Error(`Unknown ${axis} value: ${key}`);
      return [
        axis,
        choices.filter((v) => keys.has(canonicalParameters(v))),
      ] as const;
    });
    return {
      evals,
  ...
  return {
    ...
    count(selection) {
      ...
      const count = selected.axes.reduce(
        (n, [, choices]) => n * choices.length,
        selected.evals.length,
      );
