...
const evaluation: EvalDefinition = {
  id: 'task',
  agent: { async start() { return { async send() {}, async close() {} }; } },
  transcript: [], scoring: [],
};
const id = 'test-matrix';
test('matrix selects before expansion and rejects unknown/duplicate dimensions', () => {
  const m = defineEvalMatrix({ id, evals: [evaluation], parameters: { model: ['a', 'b'], mode: ['x', 'y'] }, defaults: { maxTokens: 4 } });
  ...
  expect(() => defineEvalMatrix({ id, evals: [evaluation], parameters: { x: [1, 1] } })).toThrow();
});
test('keys are canonical and ignore display names and key insertion order', () => {
  const a = defineEvalMatrix({ id, evals: [evaluation], parameters: { a: [{ y: 1, x: 2 }], b: [3] } });
  const b = defineEvalMatrix({ id, evals: [{ ...evaluation, name: 'renamed' }], parameters: { b: [3], a: [{ x: 2, y: 1 }] } });
...
test('large matrix yields a single cell without creating the Cartesian array', () => {
  ...
  const m = defineEvalMatrix({ id, evals: [evaluation], parameters: { a: values, b: values, c: values } });
